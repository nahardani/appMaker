...
import com.company.appmaker.model.ControllerDef;
import com.company.appmaker.model.EndpointDef;
...
public class ProjectScaffolder {
    ...
    public void scaffoldToDirectory(com.company.appmaker.model.Project p, java.nio.file.Path root) throws java.io.IOException {
        ...
        if (p.getPackages()!=null) for (String s : p.getPackages()) if (s!=null) {
            String t = s.trim().toLowerCase(java.util.Locale.ROOT); if(!t.isEmpty()) unique.add(t);
        }
        for (String s : unique) {
        ...
        var ctrlDir = pkgDir.resolve("controller"); java.nio.file.Files.createDirectories(ctrlDir);
        ...
        var dtoDir  = pkgDir.resolve("dto"); java.nio.file.Files.createDirectories(dtoDir);
