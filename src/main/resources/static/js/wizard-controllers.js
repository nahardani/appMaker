...
async function aiPreview(){
    ...
    const ctrl  = document.getElementById('aiControllerName').value.trim();
    const base  = document.getElementById('aiBasePath').value.trim();
    const prompt= document.getElementById('aiPrompt').value.trim();
    if(!prompt){ alert('متن منطق/نیازمندی را وارد کنید.'); return; }

    const { token, header } = getCsrf();
    const url = location.pathname.replace(/\/controllers.*/,'') + '/ai/plan';
    ...
    const res = await fetch(url, {
        ...
        body: JSON.stringify({ model, controllerName: ctrl, basePath: base, prompt })
    });
    if(!res.ok){ alert('خطا در دریافت طرح از AI ('+res.status+')'); return; }
...
async function aiGenerate(){
    ...
    const { token, header } = getCsrf();
    const url = location.pathname.replace(/\/\/?controllers.*/,'') + '/ai/generate';
    ...
    if(!res.ok){ alert('خطا در تولید کد ('+res.status+')'); return; }
    const data = await res.json();
    renderAiResult(data);
}
